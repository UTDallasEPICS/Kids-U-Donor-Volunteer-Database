...
export default async function PrintAllBackgroundChecksPage() {
  ...
  return (
    <html>
      <head>
        <title>Pending Background Checks — All</title>
        <style>{`
          body { font-family: Arial, sans-serif; color: #111; margin: 0; padding: 0; }
          .record { max-width: 800px; margin: 0 auto; padding: 40px; page-break-after: always; }
          ...
          .footer { margin-top: 48px; font-size: 11px; color: #999; border-top: 1px solid #e5e7eb; padding-top: 12px; }
          @media print { .no-print { display: none !important; } }
        `}</style>
      </head>
      <body>
        <PrintTrigger />
        <PrintToolbar count={records.length} />
